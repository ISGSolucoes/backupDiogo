...
BEGIN
  ...
  IF NEW.status = 'premiada' AND OLD.status != 'premiada' THEN
    ...
    -- Buscar dados da cotação (simulando estrutura)
    SELECT jsonb_build_object(
      'fornecedor_id', NEW.fornecedor_vencedor_id,
      'fornecedor_cnpj', NEW.fornecedor_vencedor_cnpj,
      'fornecedor_nome', NEW.fornecedor_vencedor_nome,
      'projeto_id', NEW.projeto_id,
      'centro_custo', NEW.centro_custo,
      'data_entrega', NEW.data_entrega_desejada
    ) INTO cotacao_data;

    -- Buscar itens da cotação (simulando)
    SELECT jsonb_agg(
      jsonb_build_object(
        'descricao', 'Item da cotação',
        'quantidade', 1,
        'unidade', 'UN',
        'preco_unitario', COALESCE(NEW.valor_total, 0)
      )
    ) INTO itens_cotacao;
    ...
    SELECT public.criar_po_automatico(
      ...
      cotacao_data->>'fornecedor_cnpj',
      cotacao_data->>'fornecedor_nome',
      NEW.criado_por,
      cotacao_data->>'centro_custo',
      cotacao_data->>'projeto_id',
      (cotacao_data->>'data_entrega')::date,
...
-- Função para processar requisição aprovada
...
BEGIN
  ...
  IF NEW.status = 'aprovada' AND OLD.status != 'aprovada' THEN
    ...
    -- Aqui você pode adicionar lógica para verificar se deve pular sourcing
...
DROP TRIGGER IF EXISTS trigger_requisicao_aprovada ON public.requisicoes;  
