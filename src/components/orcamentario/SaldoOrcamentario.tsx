...
import type { SaldoOrcamentario as SaldoOrcamentarioType } from '@/types/orcamento';

interface SaldoOrcamentarioProps {
  centroCusto: string;
  projeto?: string;
  categoria?: string;
...
export const SaldoOrcamentario = ({ 
  centroCusto, 
  projeto, 
  categoria, 
  valorRequisicao = 0,
  className 
}: SaldoOrcamentarioProps) => {
  const { obterSaldoOrcamentario } = useControleOrcamentario();
  const [saldo, setSaldo] = useState<SaldoOrcamentarioType | null>(null);
      ...
      <Card className={className}>
        <CardContent className="p-4">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm">Dados de orçamento não disponíveis</p>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium">{centroCusto}</span>
          </div>
          {projeto && (
            <div className="flex items-center justify-between">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium">
              R$ {saldo.valor_total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </span>
          </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium">
                {saldo.percentual_usado.toFixed(1)}%
              </span>
            </div>
            <Progress 
              value={saldo.percentual_usado} 
              className="h-2"
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">Valor Reservado:</span>
            <span className="text-sm font-medium text-warning">
              R$ {saldo.valor_reservado.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium">
              R$ {saldo.valor_utilizado.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </span>
          </div>
        </div>
        ...
        {valorRequisicao > 0 && (
          <>
            <div className="border-t pt-3">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Valor desta Requisição:</span>
                  <span className="text-sm font-medium">
                    R$ {valorRequisicao.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Saldo após Requisição:</span>
                  <span className={`text-sm font-medium ${
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm">
                ...
                <p className="text-destructive/80">
                  O valor desta requisição excede o saldo disponível em 
                  R$ {Math.abs(saldoAposRequisicao).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </p>
              </div>
            </div>
          </div>
